...
export default function Keyboard() {
    ...
    const [dictionary, setDictionary] = useState<Array<string>>(dictionaryWords);
    
    useEffect(() => {
        const word = targetWords[Math.floor((Date.now() - new Date(2022, 0, 1).getTime()) / 1000 / 60 / 60 / 24)];
        console.info(`Todays word is "${word}"...`);
        setWord(word);
    }, []);
